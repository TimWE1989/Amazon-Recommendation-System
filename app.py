...
def optimize_memory(df):
    ...
    for col in result_df.columns:
        if result_df[col].dtype == 'object':
            ...
            if unique_count / len(result_df) < 0.5:  # If less than 50% unique values
    ...
    try:
        with st.spinner("Loading data..."):
            ...
            with st.spinner("Optimizing memory..."):
                meta_df = optimize_memory(meta_df)
                reviews_df = optimize_memory(reviews_df)
                
                # Show memory usage info
                meta_memory = meta_df.memory_usage(deep=True).sum() / (1024 * 1024)
                reviews_memory = reviews_df.memory_usage(deep=True).sum() / (1024 * 1024)
