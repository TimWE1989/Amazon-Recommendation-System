...
def load_recommendation_system():
    """Load recommendation system with progressive initialization for better performance."""
    st.info("Starting recommendation system initialization...")
    ...
    try:
        ...
        if not os.path.exists(meta_path) or not os.path.exists(reviews_path):
            st.error("Required data files not found. Please download them first.")
            st.stop()
        ...
        # Step 2: Optimize memory (20%)
        status_text.text("Optimizing memory usage...")
        df1 = optimize_dataframe(df1)
        df2 = optimize_dataframe(df2)
        progress_bar.progress(20)
        
        # For development or lower memory environments, take a smaller sample
        use_sample = st.session_state.get('use_sample', True)
        sample_size = st.session_state.get('sample_size', 0.1)  # 10% by default
        
        if use_sample:
            status_text.text(f"Using {sample_size*100:.0f}% sample of data for faster processing...")
            df1, df2 = subsample_data(df1, df2, sample_size)
            st.info(f"Using {len(df1)} products and {len(df2)} reviews (sample)")
        
        # Step 3: Initialize RecommendationSystem (30%)
        status_text.text("Initializing recommendation system...")
        rec_system = RecommendationSystem(df1, df2)
        progress_bar.progress(30)
        
        # Step 4: Preprocessor (50%)
        status_text.text("Preprocessing data...")
        rec_system.preprocessor = DataPreprocessor(rec_system.meta_df, rec_system.reviews_df)
        processed_meta, processed_reviews = rec_system.preprocessor.preprocess_all()
        progress_bar.progress(50)
        
        # Step 5: Initialize simpler recommenders first (70%)
        status_text.text("Initializing basic recommenders...")
        
        # Start with simpler recommenders
        rec_system.popularity_recommender = PopularityRecommender(processed_meta, rec_system.reviews_df)
        rec_system.cluster_recommender = ClusteringRecommender(processed_meta)
        rec_system.itemset_recommender = FrequentItemsetRecommender(processed_meta)
        
        # Mark as initialized to allow graceful fallback
        rec_system.is_initialized = True
        progress_bar.progress(70)
        
        # Step 6: Initialize complex recommenders (90%)
        status_text.text("Initializing advanced recommenders...")
        
        # Only initialize these if not in ultra-light mode
        if not st.session_state.get('ultra_light_mode', False):
            try:
                rec_system.text_recommender = TextSimilarityRecommender(processed_meta)
                rec_system.text_recommender.initialize()
            except Exception as e:
                st.warning(f"Text recommender initialization skipped: {str(e)}")
            
            try:
                rec_system.influence_recommender = InfluentialProductsRecommender(processed_meta)
                rec_system.influence_recommender.initialize()
            except Exception as e:
                st.warning(f"Influence recommender initialization skipped: {str(e)}")
            
            # Only try CF if we have enough reviews
            if processed_reviews is not None and len(processed_reviews) > 100:
                try:
                    rec_system.cf_recommender = CollaborativeFilteringRecommender(processed_meta, processed_reviews)
                    rec_system.cf_recommender.initialize()
                except Exception as e:
                    st.warning(f"Collaborative filtering initialization skipped: {str(e)}")
        progress_bar.progress(90)
        
        # Step 7: Initialize hybrid recommender (100%)
        status_text.text("Finalizing system...")
        rec_system.hybrid_recommender = HybridRecommender(processed_meta, processed_reviews)
        rec_system.hybrid_recommender.initialize()
        
        # Complete
        progress_bar.progress(100)
        status_text.text("Recommendation system ready!")
...
def main():
    ...
    with st.sidebar:
        ...
        # Sample size slider
        use_sample = st.checkbox("Use data sample", value=st.session_state.get('use_sample', True),
                               help="Use a smaller dataset for better performance")
        
        if use_sample:
            sample_size = st.slider("Sample size (%)", 
                                  min_value=5, 
                                  max_value=100, 
                                  value=st.session_state.get('sample_size', 10) * 100,
                                  help="Percentage of full dataset to use")
            sample_size = sample_size / 100.0
        else:
            sample_size = 1.0
        
        ultra_light = st.checkbox("Ultra-light mode", 
                                value=st.session_state.get('ultra_light_mode', False),
                                help="Only initialize essential recommenders")
        
        if st.button("Apply Settings"):
            st.session_state.use_sample = use_sample
            st.session_state.sample_size = sample_size
            st.session_state.ultra_light_mode = ultra_light
    ...
    # Check if data files exist and download if needed
    meta_path = os.path.join(data_dir, "out_Meta.json")
    reviews_path = os.path.join(data_dir, "out_Appliances5.json")
    
    if not os.path.exists(meta_path) or not os.path.exists(reviews_path):
        st.title("Data Download Required")
        
        # Google Drive file IDs
        meta_file_id = "1H4V_ZimSgxnM2oD3zyUsVVh278JeC_g1"
        reviews_file_id = "1eqHSIaqMdo9c8dcUnMVSIEw28H6uAbZv"
        
        st.info("First-time setup: Downloading required data files...")
        
        # Download files
        download_from_gdrive(meta_file_id, meta_path)
        download_from_gdrive(reviews_file_id, reviews_path)
        
        if os.path.exists(meta_path) and os.path.exists(reviews_path):
            st.success("Files downloaded successfully!")
            st.button("Continue to App", on_click=lambda: st.rerun())
        else:
            st.error("Failed to download required files. Please check logs.")
        
        return
    
    # Main content
    if st.session_state.current_page == 'home':
        # Initialize recommendation system with improved loading
